...
class UcxYarnShuffleService extends YarnShuffleService() with UcxLogging {
  ...
  override protected def serviceInit(conf: Configuration) = {
    ...
    try {
      ...
      val shuffleServerRef = shuffleServerField.get(this).asInstanceOf[TransportServer]
      val port = shuffleServerRef.getPort()
      ...
      blockHandler.getAllMetrics().getMetrics().put("numRegisteredConnections",
          shuffleServerRef.getRegisteredConnections());
      val serviceMetrics =
