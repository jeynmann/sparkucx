...
final class ShuffleBlockFetcherIterator(
  ...
  private[this] val startTimeNano = System.nanoTime
  private[this] val startTime = startTimeNano / 1000000L
  ...
  private[this] var takeBegin = 0L
  private[this] var takeTotal = 0L
  private[this] var takeCount = 0L
  private[this] var fetchBegin = 0L
  private[this] var fetchTotal = 0L
  private[this] var fetchCount = 0L
  private[this] var computeBegin = startTimeNano
  private[this] var computeTotal = 0L
  /**
  ...
  private[this] def sendRequest(req: FetchRequest) {
    ...
    if (req.size > maxReqSizeShuffleToMem) {
      logInfo(s"<zzh> fetch to disk ${req.size} > $maxReqSizeShuffleToMem")
  ...
  override def hasNext: Boolean = {
    val ret = numBlocksProcessed < numBlocksToFetch
    if (!ret) {
      logInfo(s"@ST take+=$takeTotal;fetch+=$fetchTotal;compute+=$computeTotal")
      logInfo(s"@SC take+=$takeCount;fetch+=$fetchCount")
    }
    ret
  }
  ...
  override def next(): (BlockId, InputStream) = {
    ...
    while (result == null) {
      ...
      shuffleMetrics.incFetchWaitTime((stopFetchWait - startFetchWait) / 1000000L)

      computeTotal = (startFetchWait - computeBegin) / 1000
      takeTotal += (stopFetchWait - startFetchWait) / 1000
      takeCount += 1
  ...
  private def fetchUpToMaxBytes(): Unit = {
    ...
    computeBegin = System.nanoTime
    fetchTotal = computeBegin - fetchBegin
    fetchCount += 1
  }
